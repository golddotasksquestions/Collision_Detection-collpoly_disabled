...
func _process(_delta):
	#print (state)
	var labeltext = str("coll disabeled: ",collpol.disabled)
	label.set_text(labeltext)
	var label2text = str("monitoring: ",attarea.monitoring)
	label2.set_text(label2text)
	var label3text = str("monitorable: ",attarea.monitorable)
	label3.set_text(label3text)
	
	if collpol.disabled == false:
